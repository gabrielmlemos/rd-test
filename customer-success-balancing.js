...
function customerSuccessBalancing(
  ...
  const availableCss = customerSuccess.filter(
    (css) => !customerSuccessAway.includes(css.id) && css.score > 0
  );

  const sortedCss = availableCss
    .sort((a, b) => {
  ...
  const obj = sortedCss.reduce((prevObj, css) => {
    return {
      ...prevObj,
      [css.id]: customers.reduce((prev, customer) => {
        if (
          customer.score <= css.score &&
          !Object.values(prevObj)
            .flatMap((cst) => cst)
            .includes(customer)
        ) {
  ...
  const sortedResult = Object.entries(obj)
    .sort((a, b) => {
  ...
  if (
    ...
    sortedResult[0][1].length !== sortedResult[1][1].length
  ) {
    return parseInt(sortedResult[0][0]);
  }
